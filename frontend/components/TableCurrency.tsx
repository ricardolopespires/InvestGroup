...
import {
  ...
  const TableCurrencies = ({ currencies = [] }) => {
    ...
    // Memoize sorted currencies and total pages
    const { sortedCurrencies, totalPages } = useMemo(() => {
    ...
    // Memoize paginated currencies
    const paginatedCurrencies = useMemo(
      () =>
        sortedCurrencies.slice(
      ...
      <Card>
        ...
        <CardContent>
          ...
          {currencies?.length === 0 ? (
            ...
            paginatedCurrencies.map((item, i) => (
              <a
