...
const IsActiveRobo: React.FC<IsActiveRoboProps> = ({ roboId }) => {
  const [toggle, setToggle] = useState<boolean>(false);
  const [loading, setLoading] = useState<boolean>(false);

  useEffect(() => {
    const fetchData = async () => {
      const res = await getDetailAdvisors({ AdvisorId: roboId });    
  ...
  return (
    <div
      className={cn(
        ...
        toggle ? "bg-green-400 justify-end border-green-400" : "bg-white justify-start",
