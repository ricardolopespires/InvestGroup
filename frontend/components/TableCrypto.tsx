...
const TableCrypto = ({ crypto = [] }) => {
  const [filteredCrypto, setFilteredCrypto] = useState([]);
  const [performance, setPerformance] = useState(1);

  // Handle sorting when crypto or performance changes
  useEffect(() => {
    if (crypto.length > 0) {
      ...
      setFilteredCrypto(sortedcrypto);
    }
  }, [crypto, performance]);

  console.log(crypto);
  // Handle filtering when crypto changes
    ...
    <Card>
      ...
      <CardContent>
        ...
        <ScrollArea className="h-[400px]">
          {crypto?.map((item, i) => (
            <a href={`/investments/cryptos/${item.id}`} key={i}>
              <div
