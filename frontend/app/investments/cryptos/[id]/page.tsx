...
import { getAssetCrypto } from '@/lib/actions/actions.crypto';
...
const page = () => {
  ...
  const [error, setError] = useState(false);
    ...
    useEffect(() => {
      const fetchCryptos = async () => {
        try {
          const res = await getAssetCrypto({id:id});
          setCrypto(res);
        } catch (err) {
          setError(true);
          console.error(err);
  ...
  return (
    ...
    <div className='flex items-center justify-between gap-4 mb-4'>
      ...
      <div >
        <TableOperations symbol={`${crypto.symbol}usd`.toUpperCase()} UserId={user.email}/>
      </div>
      <div className='mb-16'>
        <TrendFollowing symbol={`${crypto.symbol}usd`.toUpperCase() } Assest={"crypto"} UserId={user.email}/>
      </div>
